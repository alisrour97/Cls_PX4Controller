...
def cost(trajectory: PiecewiseSplineTrajectory, ODE: JitParam, num=100, index=None):
    ...
    time = ODE.time_points
    indices = np.where(np.isin(time, trajectory.waypoints_t[index]))[0]

    # penalty = norm(states[indices, :3] - trajectory.waypoints[index, 0, :3])

    # PI1 = np.reshape(states[indices, ODE.states_indices["PI"]], (-1, N_p))
    PI1 = np.reshape(states[indices, ODE.states_indices["PI"]], (-1, N_p))

    PI1 = PI1[:3, :]

    A = PI1 @ c.W_range @ PI1.T
    eigvals, eigvecs = np.linalg.eig(A)  # get eigenvalues of this kernel matrix
    eigvals, eigvecs = eigvals.real, eigvecs.real

    return np.sqrt(np.max(eigvals))
    # return np.sqrt(np.max(eigvals)) + 0.04 * penalty



# This function is just for the opt of the control points
def sensitivity_optimisation(model: Model, trajectory: PiecewiseSplineTrajectory, nonlcon, target_point=None, index=None,
    ...
    if "grad" in ODE._modulename.split("_"):
        ...
        # opt = nlopt.opt(nlopt.LN_BOBYQA, len(x_ini))
    ...
    def nlc(result, x, grad):
        ...
        indice = np.where(np.isin(time_vector, trajectory.waypoints_t[index]))[0]
        result[len(cond1):] = model.eq_constraints(grad, states, time_vector, target_point, indice)
        return result[:]
    ...
    def _cost(x, grad):
        ...
        last_x = x
        trajectory.update_from_flat_params(x)
        PI = cost(trajectory, ODE, num=N, index=index)
    ...
    #         indice = np.where(np.isin(times_vec, trajectory.waypoints_t[index]))[0]
    ...
    # indexx = np.argmin(cost_list)
    indexx = -1


    print(
        f"PI improvement: {PI_init_cost:.2e} --> {cost_list[indexx]:.2e} in {T_PI} with exit code "
        f"{opt.last_optimize_result()} in {opt.get_numevals()} ({cost_eval_count}) evals"
    )

    PI_a_opt = param_list[indexx, :]
    ...
    return PI_out, cost_list[indexx], cost_list, T_PI



def sensitivity_joint_optimisation(model: Model, trajectory: PiecewiseSplineTrajectory, nonlcon, target_point=None, index=None,
    ...
    x_ini_gains = remove_gains_values(gains)
    x_ini = trajectory.get_flat_free_params()
    ...
    def nlc(result, x_t, grad):
        if np.any(x_t != last_x):
            ...
            G = x_t[len(x_ini):]
            ODE["Array_gains"] = add_gain_values(G)
    ...
    def _cost(x_t, grad):
        ...
        x = x_t[:len(x_ini)]
        trajectory.update_from_flat_params(x)
        G = add_gain_values(x_t[len(x_ini):])
        ODE["Array_gains"] = G
        ODE.apply_parameters()
    ...
    return PI_out, cost_list[index], cost_list, T_PI
