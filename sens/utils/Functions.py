...
def extract_number(filename):
    # Extract the numeric part of the filename (assuming it's before the '.ulg')
    name_without_extension = filename.rsplit('.', 1)[0]
    numeric_part = name_without_extension.lstrip('file')  # Adjust the prefix as needed
    return int(numeric_part)

def remove_gains_values(arr):
    indexes = [0, 3, 6, 10]  # List of indexes to remove
    indexes_to_remove = sorted(indexes, reverse=True)  # Sort in reverse order to avoid index shifting
    new_arr = arr.copy()
    for index in indexes_to_remove:
        if 0 <= index < len(new_arr):
            new_arr = np.delete(new_arr, index)
        else:
            print(f"Index {index} is out of range.")
    return new_arr


def add_gain_values(arr):
    new_arr = arr.copy()
    indexes = [7, 5, 3, 1]  # List of indexes to add
    for index in indexes:
        if 0 <= index < len(new_arr):
            new_value = new_arr[index - 1]
            new_arr = np.insert(new_arr, index, new_value)
        else:
            print(f"Index {index} is out of range.")
    return new_arr
...
#         sens = PI[i + 1, :, :]  # get the sensitivity matrix for this specific time
#         mat = sens @ c.W_range @ sens.T  # kernel matrix of the sensitivity
#         eigvals, eigvecs = np.linalg.eig(mat)  # get eigenvalues of this kernel matrix
#         eigvals, eigvecs = eigvals.real, eigvecs.real
#         # eigvecs, _, _ = np.linalg.svd(mat)  # get svd decomposition to construct projector
#
#         threshold = 1e-1 * np.max(eigvals)  # used to determined if an eigenvalue is considered 0 or not
#         indices_nz = []  # empty list to be filled by indices of non-zero eigen values
#         eigvec_nz = []  # empty list to be filled with eigen vectors of non-zero eigen values
#         eigenval_inv = []
#
#         for index, eigval in enumerate(eigvals):  # comparing eigen values to the threshold
#
#             if np.abs(eigval) > threshold:
#                 indices_nz.append(index)
#                 eigvec_nz.append(np.expand_dims(eigvecs[:, index], 1))
#                 eigenval_inv.append(1 / eigval)  # if value large enough
#             else:
#                 eigenval_inv.append(0)  # if the value is too low, the invert is chosen as 0
#
#         d = len(indices_nz)  # number of non-zero eigen values (used for the projector)
#         uk = eigvec_nz[0]  # initialize with first eigen vector with non-zero eigen value
#
#         for idx in range(d - 1):
#             uk = np.hstack((uk, eigvec_nz[idx + 1]))  # stack eigen vectors that have non-zero eigenvalues
#
#         proj = uk @ uk.T  # construct the projector for the range space of the kernel matrix
#
#         for k in range(N_dim):  # each state
#             tmp_radius = 0
#             for idx in range(d):  # loop for sum
#                 tmp_radius += np.dot(uk[:, idx], ei[:, k]) ** 2 * eigenval_inv[
#                     indices_nz[idx]]  # sum under the square root
#             ri = ei[:, k].T @ proj @ ei[:, k] / np.sqrt(tmp_radius)  # eq. (12)
#             data_plus_r[i + 1, k] = states_vec[i + 1, k] + ri  # upper part of tube
#             data_min_r[i + 1, k] = states_vec[i + 1, k] - ri  # lower part of tube
#
#     return data_plus_r, data_min_r
...
def tube_inputs(states_vec, model):
    ...
    return u_plus_r,  u_min_r, np.array(list_r)
...
def draw_ellipse(target, targets, axis, ep):
    ...
    # axis.plot_surface(x, y, z, color='r', linewidth=0.2, alpha=0.25, shade=True)
    axis.scatter(targets[0, 0], targets[0, 1], targets[0, 2], color='red', alpha=0.15, label='Final position $r(t_w)$')