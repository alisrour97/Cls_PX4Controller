...
class PiecewiseSplineTrajectory:
    ...
    def plotting(self):
        ...
        n = int(c.Tf/c.Ts * (1/c.N_pieces))+1

        tmp_list = []  # Initialize an empty list

        for a, ti, tf in self.traj_iter():
            a = a.reshape((self.N_dim, -1))
            a = np.reshape(a, (a.size // self.N_coeffs, self.N_coeffs))
            tmp = discrete_traj(a, self.N_coeffs, N=n)
            tmp_list.append(tmp[:, :-1].T)  # Append each tmp array to the list
            # axis.plot3D(*tmp)


        self.pos_all = np.vstack(tmp_list)  # Vertically stack all the tmp arrays
        L = len(self.pos_all)
        t = np.linspace(0, c.Tf, L)

        tmp_v = np.gradient(self.pos_all, t, axis=0)
        for i in range(np.shape(self.pos_all)[1]):
            tmp_v[:, i] = signal.savgol_filter(tmp_v[:, i], window_length=window_v, polyorder=order, mode="nearest")

        self.vel_all = tmp_v

        tmp_a = np.gradient(self.vel_all, t, axis=0)

        for i in range(np.shape(self.pos_all)[1]):
            tmp_a[:, i] = signal.savgol_filter(tmp_a[:, i], window_length=window_a, polyorder=order, mode="nearest")

        self.acc_all = tmp_a
        # axis.plot3D(pos_all[:, 0], pos_all[:, 1], pos_all[:, 2])
        # axis.plot3D(vel_all[:, 0], vel_all[:, 1], vel_all[:, 2])
        # axis.plot3D(self.acc_all[:, 0], self.acc_all[:, 1], self.acc_all[:, 2])
        # plt.show()
    ...
    def optimize(self, model, nlc, target_point, index=None, use_target=False, lower_bounds=None, upper_bounds=None, optim_time=10.0, N = 500, dx_init=0.1):
        ...
        def objective(x, grad):
            ...
            cost = _pre_opti_cost_from_states(model, states, model.ODE.time_points, target_point, self.waypoints_t[index])
        ...
        def nlc_wrap(result, x, grad):
            ...
            ######## used to be equality constrains ############

            states = model.ODE.last_result
            times_vec = model.ODE.time_points
            indice = np.where(np.isin(times_vec, self.waypoints_t[index]))[0]
            result[len(cond1):] = model.eq_constraints(grad, states, times_vec, target_point, indice)
        ...
        #     times_vec = model.ODE.time_points
        #     indice = np.where(np.isin(times_vec, self.waypoints_t[index]))[0]
        #     result[:] = model.eq_constraints(grad, states, times_vec, target_point, indice)
        #     return result
        ...
        # opt = nlopt.opt(nlopt.GN_ISRES, len(x_ini))  # choosing optimizer for pre-conditionning
        ...
        opt.add_inequality_mconstraint(nlc_wrap, model.nonlcon_tol_init())
        ...
        self.update_from_flat_params(param_list[index, :])



        print(
            f"Obj improvement: {init_cost:.2e} --> {cost[index]:.2e} "
            f"in {time.time() - tic} with exit code {opt.last_optimize_result()} in {opt.get_numevals()} {N_eval} evals"
        )
        return cost, time.time() - tic
    ...
    # def _pre_opti_cost_from_states(self, model: Model, states, time_vec, target, time_wp):
    #     c = constants()
    #     self.Construct_trajectory()
    #     L = len(self.acc_all)
    #     t = np.linspace(0, c.Tf, L)
    #     jerk = np.gradient(self.acc_all, t, axis=0)  # jerk
    #
    #     cost_jerk = 0
    #     for i in range(np.shape(jerk)[0]):
    #         cost_jerk += np.linalg.norm(jerk[i, :])
    #
    #     return cost_jerk
...
def _pre_opti_cost_from_states(model: Model, states, time_vec, target, time_wp):
    ...
    # kc = 0.02
    # k_target = 38
    k_target = 1  # 1 is working fine + cost1 + cost2

    indice = np.where(np.isin(time_vec, time_wp))[0]

    distance_to_target = norm(states[indice, :3] - target[0, :3])
    ...
    # cost = cost1 + cost2
    # It was cost2
    # return cost + k_target * distance_to_target

    # return cost2 + 22000 * distance_to_target
