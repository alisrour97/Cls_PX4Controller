...
with open('radius.csv', 'r') as csv_file:
    ...
    for row in csv_reader:
        # Assuming column1 is in index 0 and column2 is in index 1
        value_column1 = row[0]
        value_column2 = row[1]
        value_column3 = row[2]
        value_column4 = row[3]
        ...
        init_values.append(value_column1)
        init_n_values.append(value_column2)
        pi_values.append(value_column3)
        pi_n_values.append(value_column4)

# Convert lists to NumPy arrays
init_t= np.array(init_values)
init_n_t= np.array(init_n_values)
pi_t = np.array(pi_values)
pi_n_t = np.array(pi_n_values)
# Convert init_t to a numeric array
init_t = np.array(init_t, dtype=float)
# Convert init_t to a numeric array
init_n_t = np.array(init_n_t, dtype=float)
# Convert init_t to a numeric array
pi_t = np.array(pi_t, dtype=float)
# Convert init_t to a numeric array
pi_n_t = np.array(pi_n_t, dtype=float)
...
figure_2 = plt.figure(figsize=(16, 8))
...
for k in range(len(dir_name)):
    ...
    for filename in sorted_ulg_files:
        ...
        u_motor = u_motor[index_low:index_high, :]
        ...
        if counter2 == 0 and k == 0:
            for m in range(len(init_t)):
                value1.append(init_t[m] + init_n_t[m])
                value2.append(pi_t[m] + pi_n_t[m])

            value1 = np.array(value1)
            value2 = np.array(value2)
            t = np.linspace(0, u_motor[-2, 0], len(init_t))

            axis_2.plot(t, value1, 'k--', linewidth=4, label=r'Tube upper bound')
            counter2 = 1
        elif counter2 == 0 and k == 1:
            for m in range(len(init_t)):
                value1.append(init_t[m] + init_n_t[m])
                value2.append(pi_t[m] + pi_n_t[m])
            ...
            value2 = np.array(value2)
            t = np.linspace(0, u_motor[-2, 0], len(init_t))
            axis_2.plot(t, value2, 'k--', linewidth=4, label=r'Tube upper bound')
            # axis_2.plot(t, init_n_t, 'k--', linewidth=3, label=r'tube upper bound')
            counter2 = 1



        axis_2.axhline(y=1, linestyle='--', color='r', linewidth=4, alpha=0.3)
        axis_2.axvline(x=Fixed_time / 2, linestyle='--', color='b', linewidth=2)
        axis_2.set_ylim(0.5, 1.2)
        axis_2.yaxis.grid(True)

        # Increase tick label font size for axis_2
        axis_2.tick_params(axis='both', which='major', labelsize=24)  # Adjust the labelsize as needed
        ...
        if filename == "1.ulg": # for nominal case plotting
            ...
            axis_2.plot(u_motor[:-1, 0], u_motor[:-1, 2], 'r', linewidth=3,  label=r'$p = p_c$ Nominal actuator')
        else:
            axis_1.plot3D(pos[:, 1], pos[:, 2], pos[:, 3], 'g--', linewidth=1, label=r'$p \neq p_{c}$ Controller tracking with pertubation')
            if max(u_motor[:-1, 2]) == 1:
                print(filename)
                axis_2.plot(u_motor[:-1, 0], u_motor[:-1, 2], color='orange', linestyle='-', linewidth=1.5, label=r'$p \neq p_{c}$ Perturbed actuator saturated ')
            else:
                axis_2.plot(u_motor[:-1, 0], u_motor[:-1, 2], 'g--', linewidth=1.5, label=r'$p \neq p_{c}$ Perturbed actuator ')
            if k == 1 and counter == 0 and filename=='3.ulg':
                ...
                axis_2.axhline(y=1, linestyle='--', color='r', linewidth=4, label='Maximum speed')
                axis_2.legend(loc='upper center', fancybox=True, ncol=3, bbox_to_anchor=(0.48, 0.23), fontsize=24)
                counter = 1
        ...
        # axis_1.set_xlim(-3, 2.5)
        # axis_1.set_ylim(-3, 2.5)
        # axis_1.set_zlim(0, 2)
...
plt.close(figure_1)
figure_2.savefig(save_path + '/inputs_traj2_exp'+ '.pdf')
