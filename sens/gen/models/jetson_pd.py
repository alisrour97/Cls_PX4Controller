...
class Jetson(Model):
    ...
    def nonlcon(self, grad, states_vec, time_vec, umin, umax, r_min):
        ...
        c0 = np.block([n_max - umax, umin - n_min])
        return c0

    def nonlcon_init(self, grad, states_vec, time_vec, umin, umax, r_min):
        u_int = states_vec[:, self.ODE.states_indices["u_int"]]  # last 2 states are the integral of u
        u_vec = np.diff(u_int, axis=0).T / np.diff(time_vec, axis=0).T

        if grad is not None and grad.size > 0:
            raise NotImplementedError("Will see later for gradient based algo")
        else:
            n_max = np.linalg.norm(u_vec, ord=np.inf, axis=1)
            n_max = np.max(n_max)
            n_min = np.min(u_vec, axis=1)
            n_min = np.min(n_min)
            # r_curve_min = min_curvature_radius(states_vec[:, self.output_indices()], time_vec)
        c0 = np.block([n_max - umax, umin-n_min])
        return c0


    # def nonlcon_tol(self):
    #     return [1e-4]*N_inputs + [1e-4]*N_inputs + [1e-4]

    def nonlcon_tol(self):
        return [1e-3] + [1e-3] + [7e-2] * 3

    def nonlcon_tol_init(self):
        # return [1e-3]*N_inputs + [1e-3]*N_inputs + [5e-2] *3

        return [1e-3] + [1e-3] + [5e-2] * 3

    def eq_constraints(self, grad, states_vec, time_vec, target_point, indice):
        ...
        return states_vec[indice, :3].flatten() - target_point[0, :3].flatten()

    def eq_constraints_tol(self, target_point=None):
        # return [1e-6]*(target_point.shape[0]*3)
        return [1e-1] * (target_point.shape[0] * 1)
    ...
    def generate(self, N_lc, verbose=False, mode: Mode=Mode.NOGRAD, token="", overwrite=False) -> (JitParam):
        ...
        n_max = (1100)**2
        l_arm = 0.28
        ht = p[4] * g / (4 * p[0] * n_max) # hovering thrust (normalized)
        # ht = 0.5


        torque_max_x = abs(5.84*1e-6 * l_arm * (2 * np.sin(np.pi / 3)) * n_max)
        torque_max_y = abs(-5.84*1e-6 * l_arm * (2 * np.sin(np.pi / 6)) * n_max)
        torque_max_z = 2.0 * 0.06 * 5.84*1e-6 * n_max

        # torque_max_x = abs(1.51*1e-6 * l_arm * (2 * np.sin(np.pi / 3)) * n_max)
        # torque_max_y = abs(-1.51*1e-6 * l_arm * (2 * np.sin(np.pi / 6)) * n_max)
        # torque_max_z = 2.0 * 0.0157 * 1.51*1e-6 * n_max

        #
        # thrust_max = 4 * p[0] * n_max
        # torque_max_x = p[0] * l * 1.41421356 * n_max
        # torque_max_y = p[0] * l * 1.41421356 * n_max
        # torque_max_z = 2.0 * p[0] * p[1] * n_max


        # thrust_max = 4 * 5.84*1e-6 * n_max
        # torque_max_x = 3.42702187144932
        # torque_max_y = 1.9785919999999997
        ...
        # om_sp = 2 * Kq * se.a(q_err[0]) * se.DenseMatrix([q_err[1], q_err[2], q_err[3]])
        ...
        # helpers[u] = h = S.inv() * se.DenseMatrix([U1, U2, U3, U4])
        # helpers[u] = h = se.DenseMatrix([U1, U2, U3, U4])
        tmp = S*u
        U, Troll, Tpitch, Tyaw = tmp[0], tmp[1], tmp[2], tmp[3]
        # U, Troll, Tpitch, Tyaw = U1, U2, U3, U4

        # v = se.DenseMatrix([-Thrust, tmpp[0], tmpp[1], tmpp[2]])
        ...
        M = se.zeros(6)
        M[:3, :3] = m * I3
        M[:3, 3:] = -m * Fct.hat_map(rc)
        M[3:, :3] = m * Fct.hat_map(rc)
        M[3:, 3:] = I_mat #- m * Fct.hat_map(rc) @ Fct.hat_map(rc)

        f_b = -R_bw.transpose() * se.DenseMatrix([0, 0, m * g]) - m * Fct.hat_map(w) @ Fct.hat_map(w) * rc + se.DenseMatrix([0, 0, U])
        tau = Tor - m * Fct.hat_map(rc) @ R_bw.transpose() * se.DenseMatrix([0, 0, 1]) - Fct.hat_map(w) @ (I_mat - Fct.hat_map(rc) @ Fct.hat_map(rc)) * w
        ...
        dg_dp = self._problem.new_sym_matrix("dg_dp", N_ctrl_states, N_par)
        dh_dp = self._problem.new_sym_matrix("dh_dp", N_inputs, N_par)
        ...
        helpers.update({
            ...
            df_du: f.jacobian(u),
            df_dp: f.jacobian(p),
            dh_dp: h.jacobian(p) + h.jacobian(q_err)*(q_err_eval.jacobian(p) + q_err_eval.jacobian(b3d) * b3d_eval.jacobian(p)),
            dg_dp: g_xi.jacobian(p) + g_xi.jacobian(q_err)*(q_err_eval.jacobian(p) + q_err_eval.jacobian(b3d) * b3d_eval.jacobian(p)),
            ...
            dg_dq: g_xi.jacobian(q) + g_xi.jacobian(q_err) * (q_err_eval.jacobian(q) + q_err_eval.jacobian(b3d) * b3d_eval.jacobian(q)),
            dg_dxi: g_xi.jacobian(xi),
            TH: dh_dq * PI + dh_dxi * PI_xi + dh_dp,
        })

        system.update({
            PI: df_dq * PI + df_du * TH + df_dp,
            PI_xi: dg_dq * PI + dg_dxi * PI_xi + dg_dp,
            TH_i: TH,
        })
        ...
        # ODE["p"] = np.array([5.84*1e-6, 0, 0, 0, 1.535])
        # ODE["p_c"] = np.array([5.84*1e-6, 0, 0, 0, 1.535])  # [kf, gx, gy, gz, m]

        ODE["p"] = np.array([5.84*1e-6, 0, 0, 0, 2.15])
        ODE["p_c"] = np.array([5.84*1e-6, 0, 0, 0, 2.15])  # [kf, gx, gy, gz, m]
        ...
        # ODE["p_aux"] = np.array([0.029125, 0.029125, 0.055225, 0.28, 9.81, 0.06]) # [Jx, Jy, Jz, l, g, ktau]
        ODE["p_aux"] = np.array([0.029125, 0.029125, 0.055225, 0.28, 9.81, 0.06])  # [Jx, Jy, Jz, l, g, ktau]
        ODE.param_alias.update({
            "Jx": ("p_aux", 1),
            "Jy": ("p_aux", 2),
        ...
        # Gains used in 1st experiment documented
        # tPv = np.array([2.55, 2.55, 5])
        # tPr = np.array([0.95, 0.95, 1])
        # tIv = delta / delta_PX4_pos * np.array([0.6, 0.6, 2])
        # tDv = delta_PX4_pos / delta * np.array([0.2, 0.2, 0])
        #
        # tKq = np.array([6.5, 6, 4])
        # tP = np.array([0.15, 0.15, 0.2])
        # tD = delta_PX4_att / delta * np.array([0.004, 0.004, 0])
        # tI = delta / delta_PX4_att * np.array([0.1, 0.1, 0.1])



        # Tomasso tunning
        # tPv = np.array([2.55, 2.55, 4])
        # tPr = np.array([1, 1, 1])
        # tIv = delta / delta_PX4_pos * np.array([0.6, 0.6, 1.8])
        # tDv = delta_PX4_pos / delta * np.array([0.2, 0.2, 0.5])

        # My tuning
        tPv = np.array([2.55, 2.55, 5])
        tPr = np.array([0.95, 0.95, 1])
        tIv = delta / delta_PX4_pos * np.array([0.6, 0.6, 2])
        tDv = delta_PX4_pos / delta * np.array([0.2, 0.2, 0.05])

        tKq = np.array([6.5, 6, 4])
        tP = np.array([0.15, 0.15, 0.2])
        tD = delta_PX4_att / delta * np.array([0.004, 0.004, 0.0001])
        tI = delta / delta_PX4_att * np.array([0.1, 0.1, 0.1])


        ODE["Array_gains"] = np.concatenate([tPv, tPr, tDv, tIv, tKq, tP, tD, tI])
        # ODE["Array_gains2"] = np.concatenate([tPv[1:], tPr[1:], tDv[1:], tIv[1:], tKq, tP, tD, tI])
